    ...
    enum class Question(val question: String, val answers: List<String>) {
        NAME("Как меня зовут?", listOf("бендер", "bender")) {
            ...
            override fun validate(answer: String): String? =
                if (!answer[0].isUpperCase()) "Имя должно начинаться с заглавной буквы\n"
                else null
        },
        ...
        MATERIAL("Из чего я сделан?", listOf("металл", "дерево", "iron", "wood", "metal")) {
            ...
            override fun validate(answer: String): String? =
                if (answer.trim().contains(Regex("\\d"))) "Материал не должен содержать цифр\n"
                else null
        },
        BDAY("Когда меня создали?", listOf("2993")) {
            override fun nextQuestion(): Question = SERIAL
            override fun validate(answer: String): String? =
                if (answer.trim().contains(Regex("\\D")))
        ...
        SERIAL("Мой серийный номер?", listOf("2716057")) {
            ...
            override fun validate(answer: String): String? =
                if (!answer.trim().contains(Regex("\\d{7}")))
